...
image_dir = os.path.expanduser("~/Desktop/Instagram")
...
with open('instagram_posts.json', 'r') as f:
    ...
    for post in posts:
        ...
        metadata = {}
        ts = datetime.utcfromtimestamp(post['timestamp'])
        metadata['date'] = ts.isoformat()
        metadata['slug'] = str(calendar.timegm(ts.timetuple()))
        metadata['archive'] = [ts.strftime("%Y-%m")]
        metadata['alturls'] = [
            "https://www.instagram.com/p/{}".format(post['code']),
        ]
        if 'tweet_id' in post:
            metadata['alturls'].append(
                "https://twitter.com/bismark/status/{}".format(post['tweet_id'])
            )
        metadata['type'] = 'photo'
        ...
        media_file = os.path.join(image_dir, post['file'])
