...
for post in [post for post in data if 'slug' not in post][:100]:
#for post in data[14:15]:
    ...
    print ts.strftime("--------------------------------------------------------------------------------")
    print ts.strftime("%Y-%m-%d")
    print "https://www.facebook.com/{}/posts/{}".format(*post['id'].split("_"))
    print u"Search string: \n{}".format(string)

    choice = raw_input("choice: ")
    if choice == "":
        continue
    slug = os.path.basename(choice.split(":", 1)[0]).split(".")[0]
    post['slug'] = slug
